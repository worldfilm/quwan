...
function is_curl_ready()
{
	return function_exists('curl_version');
}

function dbglog($msg, $title=null)
{
	if (!is_string($msg)) {
		$msg = json_encode($msg);
	}
...
{
	if (!is_curl_ready()) {
        \Log::info('PHP CURL MODULE MISSING. INSTALL: apt install php7.1-curl php7.2-curl');
		return null;
	}
	...
	if (($err) || ($httpcode !== 200)) {
        //\Log::info($res, 'curl result');
        //\Log::info(["err"=>$err, "code"=>$httpcode], 'curl error');
		return null;
	}
	return $res;
}
...
function desEncode($key, $str)
{
	if (function_exists('mcrypt_encrypt')) {
        \Log::info('Using mcrypt encode module');
		return mcrypt_desEncode($key, $str);
	}

	if (function_exists('openssl_encrypt')) {
        \Log::info('Using openssl encode module');
		return openssl_desEncode($key, $str);
	}

    \Log::info('CAN NOT USE ENCRYPTION MODULE');
	return null;
}
...
function desDecode($key, $str)
{
	...
	if (function_exists('mcrypt_encrypt')) {
        \Log::info('Using mcrypt decode module');
		return mcrypt_desDecode($key, $str);
	}

	if (function_exists('openssl_encrypt')) {
        \Log::info('Using openssl decode module');
		return openssl_desDecode($key, $str);
	}
...
{  
	...
	$str = pkcs5_pad(trim($str), $blocksize);
...
{  
	...
	return pkcs5_unpad(trim($decrypt_str));  
