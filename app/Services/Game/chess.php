...
class chess
{
    public function __construct(){
        $config_data = GameService::getGameConfig('chess');
    ...
    public function transfer( $request ){
        ...
        if($method == 'in'){
            ...
            if($result == 'success'){
                ...
                //加入到有序集合，每5秒定时处理
                //Redis::select(14);
                //Redis::zAdd('transfer_in_list_key', time(), $record_id);
            }else{
            ...
            //加入到有序集合，每5秒定时处理
            //Redis::select(14);
            //Redis::zAdd('transfer_out_list_key', time(), $record_id);
        }
        ...
        if( $code === 0 ){
            ...
            if($method == 'out'){
                ...
                //Redis::select(14);
                //Redis::zRem('transfer_out_list_key', $record_id);
            } else {
                //Redis::select(14);
                //Redis::zRem('transfer_in_list_key', $record_id);
            }
        } else {
            $transfer_status =  0;
    ...
    public function getBalance( $request ){
        ...
        //\Log::info("开元查余额接口请求URL:".$url.",响应:".$res.",错误码:".$code.",错误消息:".$msg);
        ...
        if( $code == 0 ){
            ...
            return ['status' => 101, 'message' => $msg, 'game_money' => ''];
        } else {
            return ['status' => 102, 'message' => $msg, 'game_money' => ''];
        }
    }
    ...
    public function performTransfer($user, $method, $money){
        ...
        \Log::info("开元转账接口请求URL:".$url.",订单号:".$order_id.",响应:".$res.",错误码:".$code.",错误消息:".$msg);
    ...
    public function getRecord(){
        ...
        if( $code == 0 ){
                ...
                for($i = 0; $i < $count; $i++){
                    ...
                    if(!$has){
                        ...
                        if($profit >= $tipsDataVal){
                            ...
                            $game_title = $member ? $member->game_title : '开元' . $tmp['kind_id'];
                            $data_arr = [
            ...
            //\Log::info("开元获取游戏记录接口请求URL:".$url.",响应:".$res.",错误码:".$code.",错误消息:".$msg);
            return response()->json(['status' => $code, 'message' => $msg]);
        }
    }
