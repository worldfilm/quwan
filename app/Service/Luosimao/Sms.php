<?php
namespace App\Service\Luosimao;
/**
 * User: Administrator
 * Date: 2016/5/8 0008
 * Time: 下午 2:36
...
class Sms{
    ...
    const  config = [
            'api_key' =>'788c0bdbd09330bd0f51df7598ed065a',
            'use_ssl' => false,
    ];
    ...
    public function __construct( $param =  array() ){
        ...
        if( !isset( $param['api_key'] ) || empty($param['api_key']) ){
             $this->_api_key = self::config['api_key'];
        }
    ...
    //批量发送，用于大批量发送
    public function send_batch( $mobile_list = [] , $message = '' , $time = '' ){
